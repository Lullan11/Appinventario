...
function mostrarDocumentos(lista) {
    ...
    container.innerHTML = lista.map(doc => {
        ...
        // CORRECCIÓN: Asegurar que la fecha se muestre correctamente
        let fecha = 'Sin fecha';
        if (doc.fecha_subida) {
            try {
                // Crear la fecha ajustando la zona horaria
                const fechaObj = new Date(doc.fecha_subida);
                // Añadir la compensación de zona horaria para obtener la fecha local correcta
                ...
                fecha = new Date(doc.fecha_subida).toLocaleDateString('es-ES');
            }
        }
